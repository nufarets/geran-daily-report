...
const CITY_DEFINITIONS = Object.freeze([
  ...
  ["Вознесенск", "Николаевская область", /(?<!\p{L})(?:вознесенск|вознесенськ)(?:а|е|у|ом)?(?!\p{L})/giu],
  ...
  ["Буча", "Киевская область", /(?<!\p{L})буч(?:а|и|е|у|ей|і)(?!\p{L})/giu],
  ...
  ["Шепетовка", "Хмельницкая область", /(?<!\p{L})(?:шепетовк|шепетівк)(?:а|и|е|у|ой|і)(?!\p{L})/giu],
  ...
  ["Черновцы", "Черновицкая область", /(?<!\p{L})(?:черновц(?:ы|ов|ам|ами|ах)|чернівц(?:і|ів|ям|ями|ях))(?!\p{L})/giu],
...
const REACTIVE_UAV_PATTERN = /реактивн\p{L}*\s+(?:бпл[аa]|безп[іи]лотн\p{L}*)/iu;
...
function strikeUavClauses(text) {
  return textClauses(text).filter((clause) => (
    ...
    || ((/[🛵🏍]/u.test(clause) || REACTIVE_UAV_PATTERN.test(clause))
      && !RECONNAISSANCE_UAV_PATTERN.test(clause))
  ));
}
...
function isIsolatedReactiveUavText(text) {
  ...
  return strikeClauses.length === 1
    && /^(?:⚠️?\s*)?(?:[🛵🏍]\uFE0F?\s*)?(?:1\s+)?реактивн(?:ий|ый)\s+бпла(?!\p{L})/iu.test(strikeClauses[0]);
}
